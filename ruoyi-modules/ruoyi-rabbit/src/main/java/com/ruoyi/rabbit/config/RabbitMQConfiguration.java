...
//        spring_returned_message_correlation：所退回消息的唯一标识，可用于更新自定义的消息记录表
    ...
    //订单结果普通队列配置
    public static final String SECKILL_ORDER_RES_FAIL_EXCHANGE_NAME = "seckill.ordeResFail.exchange";
    public static final String SECKILL_ORDER_RES_FAIL_QUEUE_NAME = "seckill.ordeResFail.queue";
    public static final String SECKILL_ORDER_RES_FAIL_QUEUE_ROUTING_KEY = "seckill.ordeResFail.queue.routingkey";

    // 声明交换机，默认是持久化
    @Bean("seckillOrderResExchange")
    public DirectExchange seckillOrderResExchange() {
        return new DirectExchange(SECKILL_ORDER_RES_FAIL_EXCHANGE_NAME);
    }

    // 声明队列
    @Bean("seckillOrderResQueue")
    public Queue seckillOrderResQueue() {
        return new Queue(SECKILL_ORDER_RES_FAIL_QUEUE_NAME);
    }

    // 声明队列的绑定关系
    @Bean
    public Binding seckillBinding(@Qualifier("seckillOrderResQueue") Queue queue,
                                  @Qualifier("seckillOrderResExchange") DirectExchange exchange) {
        return BindingBuilder.bind(queue).to(exchange).with(SECKILL_ORDER_RES_FAIL_QUEUE_ROUTING_KEY);
    }
